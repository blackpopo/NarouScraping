...
def get_links(url):
    ...
    titles = soup.select('h2.entry-header')
...
def get_article(links, save_dir):
    for link in links:
        ...
        #     i.replace_with("\n")
        inner = soup.select_one('div.entry-body')
        texts = inner.select('div.t_b')
        if len(texts) == 0:
            texts = inner.select('div')
        for text in texts:
...
def call(save_dir):
    url_str = 'http://sshouko.net/page-{}.html'
    for i in range(550):
